...
Actions.prototype = {
  ...
  mail: function(email, subject, templatePath, data) {
    ...
    var approveButton = `<a href="${data.webhookAddress}/?email=${email}&applicantEmail=${data.applicantEmail}&items=${data.items}&apiKey=${receiver.apiKey}&approved=true"><button>Approve</button></a>`
    var rejectButton =  `<a href="${data.webhookAddress}/?email=${email}&applicantEmail=${data.applicantEmail}&items=${data.items}&apiKey=${receiver.apiKey}&approved=false"><button>Reject</button></a>`
